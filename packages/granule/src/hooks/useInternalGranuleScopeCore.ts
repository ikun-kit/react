...
>(
  ...
  return useMemo(() => {
    return perf.measure(
      ...
      () => {
        // 创建副本避免直接修改输入数据
        const state = perf.measure('State Copy', () => [...items], {
          items: items.length,
        });

        // 创建快速查找的哈希表
        const stateMap = perf.measure(
          'State Map Creation',
          () => {
        ...
        const { observable, upwardObservableInstance } = perf.measure(
          'Observable Creation',
          () => ({
            observable: new Observable<TGranuleScopePayloadMap<K, V>>(),
            upwardObservableInstance: new Observable<U>(),
          }),
        );
        ...
        const list: TGranuleScopeCore<K, V>['list'] = {
          insert(id: K, data: V, beforeId?: K) {
            if (stateMap.has(id)) {
              throw new Error(
                `GranuleScope: item with id ${id} already exists`,
              );
            }
