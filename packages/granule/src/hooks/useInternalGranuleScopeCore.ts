...
>(
  ...
  return useMemo(() => {
    perf.point('ScopeCore useMemo');

    // 创建副本避免直接修改输入数据
    perf.point('State Copy');
    const state = [...items];
    perf.span('State Copy', { items: items.length });

    // 创建快速查找的哈希表
    perf.point('State Map Creation');
    const map = new Map<K, TGranuleScopeItem<K, V>>();
    state.forEach(item => map.set(item.id, item));
    const stateMap = map;
    perf.span('State Map Creation', { items: items.length });

    // 创建事件观察者实例，用于管理所有作用域事件
    perf.point('Observable Creation');
    const observable = new Observable<TGranuleScopePayloadMap<K, V>>();
    const upwardObservableInstance = new Observable<U>();
    perf.span('Observable Creation');
    ...
    const item: TGranuleScopeCore<K, V>['item'] = {
      /** 更新指定项目的状态数据 */
      update: (id: K, data: V) => {
        // 使用O(1)查找替代O(n)的findIndex
        const mapItem = stateMap.get(id);
        if (!mapItem) {
          console.warn(
    ...
    const list: TGranuleScopeCore<K, V>['list'] = {
      insert(id: K, data: V, beforeId?: K) {
        ...
        if (beforeId && stateMap.has(beforeId)) {
          // 只有beforeId存在时才进行O(n)查找
    ...
    perf.span('ScopeCore useMemo', { items: items.length });
    return result;
  }, [items, domRef]);
};
